...
inputs  (from ../data_raw/):
  espn_games_all.csv   — from scrape_espn.py
  espn_team_stats_all.csv — from scrape_espn.py
...
def load_and_clean():
    ...
    schedule_path = DATA_RAW / "espn_games_all.csv"
    ...
    for p in [schedule_path, stats_path, odds_path]:
        if not p.exists():
            raise FileNotFoundError(
                f"missing file: {p}\n"
                f"make sure you ran scrape_espn.py and pull_odds_api.py first."
            )
